...
//  |> (fun g -> List.head g |> Seq.item 1) // -> Group

 //   |> (fun g -> List.head g |> Seq.item 1 |> string)
//  |> (fun g -> List.head g |> Seq.item 1 |> int |> (fun x -> if x > 5 then true else false))
    
(*
|> (fun s -> s |> int |> (fun x -> if x < 6 then false else true))
    |> List.filter (fun g ->
